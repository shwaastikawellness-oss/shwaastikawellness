...
const positions = [
  ...
  { x: 245, y: 34, scale: 0.78, rotate: 4, zIndex: 20, opacity: 0.68, blur: 1.2 },
  { x: 0, y: 84, scale: 0.66, rotate: 0, zIndex: 10, opacity: 0.34, blur: 2.4 },
  { x: -245, y: 34, scale: 0.78, rotate: -4, zIndex: 20, opacity: 0.68, blur: 1.2 },
];
    ...
    <section className="overflow-hidden bg-[#fbf8f1] px-5 py-16 sm:px-8 lg:px-10">
      <div className="mx-auto max-w-7xl">
        <div className="grid items-center gap-10 lg:grid-cols-[1.02fr_0.98fr]">
          <div className="relative hidden h-[510px] items-center justify-center lg:flex">
            ...
            {services.map((service, index) => {
              ...
              return (
                ...
                >
                  <div className={`flex aspect-[4/5] items-end rounded-[1.5rem] bg-gradient-to-br ${service.tone} p-5`}>
                    <div className="rounded-2xl bg-white/75 p-4 backdrop-blur">
                      <h3 className="text-2xl font-semibold text-[#2f2822]">{service.title}</h3>
                      <p className="mt-2 text-sm leading-6 text-[#66584d]">{service.intro}</p>
                    </div>
                  </div>
                </motion.button>
              );
            ...
            <div className="flex gap-4 overflow-x-auto pb-4">
              {services.map((service, index) => (
                <button
                  ...
                  type="button"
                  onClick={() => selectService(index)}
                  className={`w-[78vw] max-w-sm shrink-0 rounded-[1.75rem] border p-3 text-left shadow-lg ${
                    activeIndex === index ? "border-[#3f5f46] bg-white" : "border-[#e5d9c7] bg-white/70"
                  }`}
                >
                  <div className={`flex aspect-[4/5] items-end rounded-[1.35rem] bg-gradient-to-br ${service.tone} p-4`}>
                    <div className="rounded-2xl bg-white/75 p-4 backdrop-blur">
                      <h3 className="text-xl font-semibold text-[#2f2822]">{service.title}</h3>
                      <p className="mt-2 text-sm leading-6 text-[#66584d]">{service.intro}</p>
                    </div>
                  </div>
                </button>
          ...
          <motion.div
            layout
            className="rounded-[2rem] border border-[#e5d9c7] bg-white/85 p-7 shadow-xl shadow-[#6b513b]/8 backdrop-blur sm:p-9"
          >
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <p className="mt-4 text-lg leading-8 text-[#66584d]">{activeService.intro}</p>
                  ...
                  {showPricing ? (
                    ...
                    >
                      <div className="mt-7 rounded-[1.5rem] border border-white/60 bg-white/60 p-5 shadow-inner">
                        ...
                        {activeService.pricing.length > 0 ? (
                          <div className="space-y-5">
                            {activeService.pricing.map(([name, price, note]) => (
                          ...
                          <p className="text-base leading-8 text-[#4b423b]">{activeService.details}</p>
                        )}
                      </div>
                    </motion.div>
                  ) : null}
